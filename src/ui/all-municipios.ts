...
export interface AllMunicipios {
  mount(onSelect: (m: Municipio, entry: CamIndexEntry | null) => void): Promise<void>;
...
export function createAllMunicipios(root: HTMLElement): AllMunicipios {
  ...
  function renderList(query: string, onSelect: (m: Municipio, e: CamIndexEntry | null) => void): void {
    ...
    if (stat) stat.textContent = `${matched.length} de ${entries.length}`;
