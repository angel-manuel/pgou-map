...
import { createMap, installLayers, setActiveCamSlugs, setColorMode, setDormidos, setReconvertible, setSoloMode, setZones, } from "./map";
...
async function main() {
    ...
    async function applyZones() {
        ...
        const fcs = await Promise.all([...selected.values()].map(async (m) => {
    ...
    async function applyDerived() {
        ...
        if (layerState.dormidos || layerState.reconvertible) {
            await Promise.all([...selected.values()].map(async (m) => {
                if (layerState.dormidos) {
                    const d = m.derived.find((x) => x.id === "sectores-dormidos");
                    if (d)
                        dormidosFiles.push(await loadGeoJSON(d.file));
                }
                if (layerState.reconvertible) {
                    const r = m.derived.find((x) => x.id === "reconvertible");
                    if (r)
                        reconvFiles.push(await loadGeoJSON(r.file));
                }
            }));
        }
        setDormidos(map, layerState.dormidos ? mergeFCs(dormidosFiles) : null, layerState.dormidos);
        setReconvertible(map, layerState.reconvertible ? mergeFCs(reconvFiles) : null, layerState.reconvertible);
    }
    function refreshUI() {
    ...
    async function onSelectionChanged(newSelected) {
        ...
        if (selected.size > 0) {
            ...
            if (bounds) {
                if (selected.size === 1) {
                    const m = selected.values().next().value;
